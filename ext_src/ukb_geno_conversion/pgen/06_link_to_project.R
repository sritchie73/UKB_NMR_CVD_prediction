...
common_dir=sprintf("%s/genetics/COMMON/post_qc_data/imputed/HRC_UK10K/plink_format/GRCh37/pgen", rds_dir)
...
for (proj_id in projects) {
  target_dir = sprintf("%s/genetics/%s/post_qc_data/imputed/HRC_UK10K/plink_format/GRCh37/pgen", rds_dir, proj_id)
  dir.create(target_dir, recursive=TRUE, showWarnings=FALSE)
  sample = fread(sample_files[proj_id])[-1]
  sample[, row := .I]
  for (chr_id in c(1:22, "X", "XY")) {
    system(sprintf("ln -s %s/ukb_imp_v3_dedup_chr%s.pgen %s/", common_dir, chr_id, target_dir), wait=TRUE)
    system(sprintf("ln -s %s/ukb_imp_v3_dedup_chr%s.pvar %s/", common_dir, chr_id, target_dir), wait=TRUE)
    psam = fread(sprintf("%s/ukb_imp_v3_dedup_chr%s.psam", common_dir, chr_id))
    psam[sample, on = .(IID=row), c("#FID", "IID") := .(ID_1, ID_2)]
    fwrite(psam, sep="\t", quote=FALSE, file=sprintf("%s/ukb_imp_v3_dedup_chr%s.psam", target_dir, chr_id))
  }
