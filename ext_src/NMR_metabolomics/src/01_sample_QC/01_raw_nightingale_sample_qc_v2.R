...
# Resolve duplicate plate+position measurements and dupliate sample_id + visit 
# measurements dropping one from each pair and note the reason for this: 
#  - "Different spectrometer to rest of plate", 
#  - "Duplicate sample with largest time difference to rest of plate", # Resolved in phase 1 release
#  - "Duplicate sample with longest time between preparation and measurement"
# 
# Apart from the first reason, the other samples (N=10) are read in from a
# redacted file, as all are identified in part by sample_id. UK Biobank policy 
# is that sample IDs cannot be included in published source code as they may 
# potentially be used to re-identify participants.  
sinfo[plate_id == "0490000006107" & spectrometer == "10176949", c("removed", "removal_reason") := .(TRUE, "Different spectrometer to rest of plate")]
to_rm <- fread("ext_src/NMR_metabolomics/src/01_sample_QC/samples_failing_qc.csv")
sinfo[to_rm, on = .(sample_id, plate_id), c("removed", "removal_reason") := .(TRUE, i.reason)]
