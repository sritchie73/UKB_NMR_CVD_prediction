...
message(sprintf("Training assay score for %s in %ss in prediction test fold %s\n", this_endpoint, this_sex, this_test_fold))

# Make output directory
out_dir <- sprintf("analyses/assay_score_training/test_fold_%s/%s/%s", this_test_fold, this_endpoint, this_sex)
...
# Add in standardized assay concentratiosn
assay_scaled <- fread("data/standardised/assay_concentrations.txt")
dat <- dat[assay_scaled, on = .(eid), nomatch=0]
...
training_runs <- foreach(this_type = c("assays")) %do% {
  # Get list of candidate biomarkers
  biomarkers <- names(assay_scaled)[-1]
...
fitdt <- foreach(this_type = c("assays"), .combine=rbind) %:% 
