...
nri_estimates <- rbindlist(idcol="sex", fill=TRUE, lapply(nri_lists, function(l1) {
  rbindlist(idcol="model_comparison", fill=TRUE, lapply(l1, function(l2) {
    rbindlist(idcol="age_group", fill=TRUE, lapply(l2, function(l3) {
...
reclassified[reclassified_cases, on = .(sex, model_comparison, age_group, Standard, New), Cases := N]
setnames(reclassified, "Standard", "Old")

# Add in total sample size and total cases to reclassified table
reclassified[nri_estimates, on = .(sex, model_comparison, age_group), Total_Samples := i.samples]
reclassified[nri_estimates, on = .(sex, model_comparison, age_group), Total_Cases := i.cases]
...
g <- ggplot(ggdt) +
  ...
  geom_errorbarh(data=ggdt[metric == "NRI-"], height=0, position=position_nudge(y=0.075), show.legend=FALSE) +
