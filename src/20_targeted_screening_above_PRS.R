...
# cholesterol which are dropped for NMR biomarker models), PRS, and LDL cholesterol
...
test <- test[!is.na(ldl) & !is.na(hdl) & !is.na(tchol) & !is.na(CAD_metaGRS)]
...
pred <- foreach(midx = model_info[,.I], .combine=rbind) %do% {
  ...
  # Skip models without PRS and model with conventional risk factors alone
  if (!(this_model$PGS) || this_model$name == "Conventional RF") return(NULL)
...
# (conventional risk factors plus PRS, in the subset of individuals with complete data on both
...
# Write out
fwrite(ons_conv_rf, sep="\t", quote=FALSE, file=sprintf("%s/ONS_conv_rf_prs_risk_stratified_by_age_sex.txt", out_dir))
fwrite(ons_conv_rf_summary, sep="\t", quote=FALSE, file=sprintf("%s/ONS_conv_rf_prs_risk_stratified.txt", out_dir))
...
fwrite(ukb_conv_rf, sep="\t", quote=FALSE, file=sprintf("%s/UKB_conv_rf_prs_risk_stratified_by_age_sex.txt", out_dir))
fwrite(ukb_conv_rf_summary, sep="\t", quote=FALSE, file=sprintf("%s/UKB_conv_rf_prs_risk_stratified.txt", out_dir))
...
# Write out
fwrite(ons_pred, sep="\t", quote=FALSE, file=sprintf("%s/ONS_targeted_biomarker_risk_stratified_by_age_sex.txt", out_dir))
fwrite(ons_pred_summary, sep="\t", quote=FALSE, file=sprintf("%s/ONS_targeted_biomarker_risk_stratified.txt", out_dir))
...
fwrite(ukb_pred, sep="\t", quote=FALSE, file=sprintf("%s/UKB_targeted_biomarker_risk_stratified_by_age_sex.txt", out_dir))
fwrite(ukb_pred_summary, sep="\t", quote=FALSE, file=sprintf("%s/UKB_targeted_biomarker_risk_stratified.txt", out_dir))
...
case_treat_prevent[, treatment_reason := fcase(
  treatment_reason == "conv_rf_treat", "High risk, blanket screening with conventional risk factors plus PRS", 
  treatment_reason == "intermed_treat", "Medium risk, diabetic or LDL-C >= 5 mmol/L",
  treatment_reason == "pred_treat", "Medium risk reclassified as high risk with targeted assessment of biomarkers"
)]
