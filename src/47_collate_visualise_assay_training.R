...
fitdt <- foreach(this_test_fold = 1:5, .combine=rbind) %do% {
  foreach(this_endpoint = c("CAD", "Stroke"), .combine=rbind) %do% {
    foreach(this_sex = c("Female", "Male"), .combine=rbind) %do% {
      model_dir <- sprintf("analyses/assay_score_training/test_fold_%s/%s/%s", this_test_fold, this_endpoint, this_sex)
...
fwrite(fitdt, sep="\t", quote=FALSE, file="analyses/assay_score_training/all_elasticnet_fits.txt")
fwrite(bestfit, sep="\t", quote=FALSE, file="analyses/assay_score_training/best_fits_per_alpha.txt")
fwrite(bestbestfit, sep="\t", quote=FALSE, file="analyses/assay_score_training/best_fits.txt")
fwrite(bestcoef, sep="\t", quote=FALSE, file="analyses/assay_score_training/best_fits_coef.txt")
fwrite(avgbestcoef, sep="\t", quote=FALSE, file="analyses/assay_score_training/avg_best_coefs.txt")
...
for (this_test_fold in 1:5) {
  for (this_type in c("assays")) {
    ...
    out_dir <- sprintf("analyses/assay_score_training/test_fold_%s/", this_test_fold)
