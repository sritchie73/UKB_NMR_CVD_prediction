...
out_dir <- "analyses/public_health_modelling/UK_population_generalised/staged_screening"
...
g <- ggplot(phs[lambda != "lambda.1se" & guidelines == "NICE.2014"]) + 
  ...
  scale_x_continuous(
    ...
    limits=c(2800, 4000), oob=scales::oob_keep,
...
g <- ggplot(phs[lambda != "lambda.1se" & guidelines == "NICE.2014"]) + 
  ...
  scale_x_continuous(
    ...
    limits=c(560, 800), oob=scales::oob_keep,
...
g <- ggplot(phs[lambda != "lambda.1se" & guidelines == "NICE.2014"]) + 
  ...
  scale_x_continuous(
    ...
    limits=c(15000, 23000), oob=scales::oob_keep,
