...
# Write out
fwrite(ons_pred, sep="\t", quote=FALSE, file=sprintf("%s/ONS_biomarker_prs_risk_stratified_by_age_sex.txt", out_dir))
fwrite(ons_pred_summary, sep="\t", quote=FALSE, file=sprintf("%s/ONS_biomarker_prs_risk_stratified.txt", out_dir))
...
fwrite(ukb_pred, sep="\t", quote=FALSE, file=sprintf("%s/UKB_biomarker_prs_risk_stratified_by_age_sex.txt", out_dir))
fwrite(ukb_pred_summary, sep="\t", quote=FALSE, file=sprintf("%s/UKB_biomarker_prs_risk_stratified.txt", out_dir))
...
to_fill <- rbind(idcol="risk_group",
  medium = pred[risk_group == "medium" & diabetes == FALSE & ldl < 5,
                .(cases=sum(incident_cvd), controls=sum(!(incident_cvd))),
                by=.(name, lambda, PGS, long_name, sex, age_group, guidelines)],
  high   = pred[risk_group == "medium" & (diabetes == TRUE | ldl >= 5),
                .(cases=sum(incident_cvd), controls=sum(!(incident_cvd))),
                by=.(name, lambda, PGS, long_name, sex, age_group, guidelines)]
)
