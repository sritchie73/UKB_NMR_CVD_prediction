...
bio_sinfo <- fread("data/ukb/full_ukb_biomarkers/output/samples_not_measured.txt")
...
# Add in NMR data
nmr <- fread("data/ukb/phase3_NMR_metabolomics/biomarker_measurements.txt")
...
# First, filter to just people who had NMR data quantified in phase 3
nmr_info <- fread("data/ukb/phase3_NMR_metabolomics/sample_information.txt")
nmr_info <- nmr_info[visit == "Main Phase" & phase == 3]
dat <- dat[eid %in% nmr_info$eid_30418]
dat <- dat[!(no_genetics)]
...
ggsave(g, width=6, height=4, units="in", file="data/cleaned/phase3_sample_missingness.png")
...
fwrite(miss_thresh, sep="\t", quote=FALSE, file="data/cleaned/phase3_missingness_threshold_consequences.txt")

# Get biomarker missingness reason
nmr_qc_flags <- fread("data/ukb/phase3_NMR_metabolomics/biomarker_QC_tags.txt")
nmr_qc_flags <- nmr_qc_flags[visit == "Main Phase"]
setnames(nmr_qc_flags, "eid_30418", "eid")
nmr_qc_flags <- nmr_qc_flags[eid %in% dat$eid]
...
fwrite(nmr_miss_tags, sep="\t", quote=FALSE, file="data/cleaned/phase3_missingness_reasons.txt")
...
fwrite(sample_miss_tags, sep="\t", quote=FALSE, file="data/cleaned/phase3_missingness_reason_by_sample.txt")
...
fwrite(sample_info, sep="\t", quote=FALSE, file="analyses/phase3_sample_flowchart.txt")
