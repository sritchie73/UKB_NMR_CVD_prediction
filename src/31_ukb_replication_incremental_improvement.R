...
# Load required data
cinds <- fread("analyses/test/phase3_cindices.txt")
risk <- fread("analyses/CVD_weight_training/phase3_CVD_linear_predictors_and_risk.txt")
nri <- fread("analyses/test/phase3_categorical_nri_estimates.txt")
...
g1 <- ggplot(cinds) +
  ...
  scale_x_continuous("Change in C-index (95% CI)", limits=c(0, 0.03), oob=scales::oob_keep) +
  ylab("") + 
  theme_bw() +
  theme(
...
g3 <- ggplot(nri) +
  ...
  scale_x_continuous("Categorical NRI, % reclassified (95% CI)", labels=percent, limits=c(-0.05, 0.16), oob=scales::oob_keep) +
  ylab("") +
  theme_bw() +
  theme(
    axis.text.y=element_text(size=8, color="black"), axis.title.y=element_blank(),
...
ggsave(g, width=10, height=3, file="analyses/test/phase3_incremental_improvement.pdf")
...
g <- plot_grid(g1, g2, g3, nrow=1, align="h", axis="l")
ggsave(g, width=10, height=5, file="analyses/test/phase3_incremental_improvement_sex_specific.pdf")
