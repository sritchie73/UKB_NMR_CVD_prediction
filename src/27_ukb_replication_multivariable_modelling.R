...
# Load required data
dat <- fread("data/cleaned/phase3_analysis_cohort.txt", select=c("eid", "sex", "age", "incident_cvd_followup", "incident_cvd",  "smoking", "sbp", "hdl", "tchol", "SCORE2_excl_UKB", "CAD_metaGRS", "Stroke_metaGRS"))
setnames(dat, "SCORE2_excl_UKB", "SCORE2")

# Add in test scores
test_scores <- fread("analyses/nmr_score_training/phase3_nmr_scores.txt")
setnames(test_scores, gsub("NMR", "NMR", names(test_scores)))
dat <- dat[test_scores, on = .(eid, sex)]
...
fwrite(offset_hrs, sep="\t", quote=FALSE, file="analyses/test/phase3_score_HRs_independent_of_score2.txt")
...
fwrite(multi_hrs, sep="\t", quote=FALSE, file="analyses/test/phase3_score_HRs_independent_of_score2_risk_factors.txt")
...
ggsave(g, width=7, height=3, file="analyses/test/phase3_sex_stratified_HRs.pdf")

# Construct table for supplement
dt <- ggdt[order(model), .(model, variable, HR, L95, U95, pval)]
fwrite(dt, sep="\t", quote=FALSE, file="analyses/test/phase3_sex_stratified_HRs.txt")
...
ggsave(g, width=7, height=4, file="analyses/test/phase3_sex_specific_HRs.pdf")
...
fwrite(dt, sep="\t", quote=FALSE, file="analyses/test/phase3_sex_specific_HRs.txt")
