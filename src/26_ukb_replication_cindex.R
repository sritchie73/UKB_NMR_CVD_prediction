...
# Load required data
dat <- fread("analyses/CVD_weight_training/phase3_CVD_linear_predictors_and_risk.txt")
...
model_info <- foreach(this_sex=c("Males", "Females", "Sex-stratified"), .combine=rbind) %:%
  foreach(this_model=c("SCORE2", "SCORE2 + NMR scores", "SCORE2 + PRSs", "SCORE2 + NMR scores + PRSs"), .combine=rbind) %do% {
		data.table(sex=this_sex,  model=this_model, score_type="non-derived")
}
model_info[model == "SCORE2", score_type := NA]
...
boot.fun <- function(dt) {
  model_cinds <- foreach(midx = model_info[,.I], .combine=rbind) %do% {
...
# Write out
fwrite(cinds, sep="\t", quote=FALSE, file="analyses/test/phase3_cindices.txt")
...
dt <- dt[order(model)]
fwrite(dt, sep="\t", quote=FALSE, file="analyses/test/phase3_sex_stratified_cindices_for_supp.txt")
...
fwrite(dt, sep="\t", quote=FALSE, file="analyses/test/phase3_sex_specific_cindices_for_supp.txt")
...
ggsave(g, width=3.5, height=2, file="analyses/test/phase3_cindex_sex_stratified.pdf")
...
ggsave(g, width=7.2, height=2, file="analyses/test/phase3_cindex_sex_specific.pdf")
