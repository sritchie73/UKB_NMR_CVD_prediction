...
cvd_cohort_info <- dat[order(sex),.(
  ...
  age = sprintf("%s (%s)", round(median(age), digits=1), round(sd(age), digits=1)),
  BMI = sprintf("%s (%s)", round(median(na.omit(bmi)), digits=1), round(sd(na.omit(bmi)), digits=1)),
  SBP = sprintf("%s (%s)", round(median(sbp), digits=1), round(sd(sbp), digits=1)),
  ...
  hdl_cholesterol = sprintf("%s (%s)", round(median(hdl, na.rm=TRUE), digits=1), round(sd(hdl, na.rm=TRUE), digits=1)),
  total_cholesterol = sprintf("%s (%s)", round(median(tchol, na.rm=TRUE), digits=1), round(sd(tchol, na.rm=TRUE), digits=1)),
  ldl_cholesterol = sprintf("%s (%s)", round(median(ldl, na.rm=TRUE), digits=1), round(sd(ldl, na.rm=TRUE), digits=1)),
  SCORE2 = sprintf("%s%% (%s%%)", round(median(SCORE2_uk_risk*100), digits=1), round(sd(SCORE2_uk_risk * 100), digits=1)),
  SCORE2_excl_UKB = sprintf("%s%% (%s%%)", round(median(SCORE2_excl_UKB_uk_risk * 100), digits=1), round(sd(SCORE2_excl_UKB_uk_risk * 100), digits=1)),
  CAD_metaGRS = sprintf("%s (%s)", round(median(CAD_metaGRS, na.rm=TRUE), digits=1), round(sd(CAD_metaGRS, na.rm=TRUE), digits=1)),
  Stroke_metaGRS = sprintf("%s (%s)", round(median(Stroke_metaGRS, na.rm=TRUE), digits=1), round(sd(Stroke_metaGRS, na.rm=TRUE), digits=1)),
