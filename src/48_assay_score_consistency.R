...
# Load scaling factors for assay data
assay_scaling <- fread("data/standardised/assay_scaling_factors.txt")

# Load assay score coefficients
assay_coef <- fread("analyses/assay_score_training/best_fits_coef.txt")

# Build long table of Assay coefficients and age interactions standardised using the given scaling factors
assay_long <- melt(dat, 
  id.vars=c("eid", "sex", "age", "cvd_prediction_foldid"),
  measure.vars=assay_coef[!(coef %like% "^age"), unique(coef)])
assay_long[assay_scaling, on = .(sex, variable=biomarker), value := (value - i.mean)/i.sd]
assay_long[, age := (age - 60)/5]
assay_age_interactions <- copy(assay_long)
assay_age_interactions[, value := value * age]
assay_age_interactions[, variable := paste0("age:", variable)]
assay_long <- rbind(assay_long, assay_age_interactions)

# Compute assay biomarker scores
models <- unique(assay_coef[,.(prediction_cv_testfold, endpoint, lambda.fit, type)])
scores <- foreach(midx = models[,.I], .combine=rbind) %do% {
  this_minfo <- models[midx]
  this_coef <- assay_coef[this_minfo, on = .(prediction_cv_testfold, endpoint, lambda.fit, type)]
  this_sinfo <- dat[,.(eid, sex, cvd_prediction_foldid)] 

  this_score <- assay_long[this_coef, on = .(sex, variable=coef)][, .(
...
avg_assay_coef <- fread("analyses/assay_score_training/avg_best_coefs.txt")
models <- unique(models[,.(endpoint, lambda.fit, type)])
avg_scores <- foreach(midx = models[,.I], .combine=rbind) %do% {
  this_minfo <- models[midx]
  this_coef <- avg_assay_coef[this_minfo, on = .(endpoint, lambda.fit, type), nomatch=0]
  this_sinfo <- dat[,.(eid, sex, cvd_prediction_foldid)] 
...
# Assess consistency of assay biomarker scores across training runs
...
score_scatter <- function(score_comp) {
  ...
  cor_anno <- score_comp[,.(label=sprintf("r=%.2f\np=%.2f", 
    cor(x_score, y_score, use="pairwise.complete.obs"), 
    cor(x_score, y_score, method="spearman", use="pairwise.complete.obs")
  )), by=.(x_model, y_model)]
  ...
  ggplot(score_comp) +
    ...
    xlab("Assay score") + ylab("Assay score") +
    theme_bw() +
    theme(
...
g1 <- score_scatter(score_comp[endpoint == "CAD" & sex == "Male" & lambda.fit == "lambda.min" & type == "assays" & x_model != "avg" & y_model != "avg"])
g2 <- score_scatter(score_comp[endpoint == "CAD" & sex == "Female" & lambda.fit == "lambda.min" & type == "assays" & x_model != "avg" & y_model != "avg"])
g3 <- score_scatter(score_comp[endpoint == "Stroke" & sex == "Male" & lambda.fit == "lambda.min" & type == "assays" & x_model != "avg" & y_model != "avg"])
g4 <- score_scatter(score_comp[endpoint == "Stroke" & sex == "Female" & lambda.fit == "lambda.min" & type == "assays" & x_model != "avg" & y_model != "avg"])

ggsave(g1, width=7, height=6, file="analyses/assay_score_training/CAD_male_assay_score_consistency.pdf")
ggsave(g2, width=7, height=6, file="analyses/assay_score_training/CAD_female_assay_score_consistency.pdf")
ggsave(g3, width=7, height=6, file="analyses/assay_score_training/Stroke_male_assay_score_consistency.pdf")
ggsave(g4, width=7, height=6, file="analyses/assay_score_training/Stroke_female_assay_score_consistency.pdf")
...
score_scatter2 <- function(lambda, score_type) {
  ...
  ggplot(ggdt) +
    ...
    xlab("Assay score predicted in 20% of withheld test-fold data") +
    ylab("Assay scores including training data") + 
...
g <- score_scatter2("lambda.min", "assays")
ggsave(g, width=7.2, height=6.6, file="analyses/assay_score_training/assay_score_consistency_simplified.pdf")
...
avg_vs_test_scatter <- function(lambda, score_type) {
  ...
  ggplot(score_comp[x_model == "avg" & y_model == cvd_prediction_foldid & lambda.fit == lambda & type == score_type]) +
    ...
    xlab("Assay score from coefficient averages") + ylab("Assay scores aggregated across test folds") +
    theme_bw() +
    theme(
      axis.text=element_text(size=6), axis.title=element_text(size=8),
      strip.text=element_text(size=8, face="bold"), strip.background=element_blank(),
...
g <- avg_vs_test_scatter("lambda.min", "assays")
ggsave(g, width=7, height=5.5, file="analyses/assay_score_training/aggregate_test_vs_coef_average_assay_scores.pdf")
...
full_vs_truncated_scatter <- function(lambda, score_type) {
  cor_anno <- avg_scores[lambda.fit == lambda & type == score_type,
    .(label=sprintf("r=%.2f\np=%.2f",
      cor(linear_predictor, lp_simplified, use="pairwise.complete.obs"),
      cor(linear_predictor, lp_simplified, method="spearman", use="pairwise.complete.obs")
    )), by=.(endpoint, sex)]
  ...
  ggplot(avg_scores[lambda.fit == lambda & type == score_type]) +
    ...
    geom_text_npc(data=cor_anno, npcx="left", npcy="top", aes(label=label), color="red", size=6*0.352777778) +
    facet_grid2(fct_rev(sex) ~ endpoint, scales="free", independent="all") +
    xlab("Average assay score from full precision coefficients") + 
    ylab("Average assay score from coefficients rounded to 3 decimal places") +
    theme_bw() +
    theme(
      axis.text=element_text(size=6), axis.title=element_text(size=8),
      strip.text=element_text(size=8, face="bold"), strip.background=element_blank(),
...
g <- full_vs_truncated_scatter("lambda.min", "assays")
ggsave(g, width=7, height=5.5, file="analyses/assay_score_training/full_coef_vs_truncated_coef_assay_scores.pdf")

# Write out full scores including training data
full_scores <- scores[lambda.fit == "lambda.min" & type == "assays"]
full_scores <- full_scores[,.(eid, sex, cvd_prediction_foldid, prediction_cv_testfold, endpoint, linear_predictor)]
fwrite(full_scores, sep="\t", quote=FALSE, file="analyses/assay_score_training/assay_scores.txt")

# Write out aggregate scores
test_scores <- scores[lambda.fit == "lambda.min" & prediction_cv_testfold == cvd_prediction_foldid & type == "assays"]
test_scores <- dcast(test_scores, eid  ~ endpoint, value.var="linear_predictor")
setnames(test_scores, c("CAD", "Stroke"), c("CAD_assay_score", "Stroke_assay_score"))
fwrite(test_scores, sep="\t", quote=FALSE, file="analyses/assay_score_training/aggregate_test_assay_scores.txt")

# Also average scores for downstream checks
avg_scores_wide <- dcast(avg_scores[lambda.fit == "lambda.min" & type == "assays"], eid ~ endpoint, value.var="linear_predictor")
setnames(avg_scores_wide, c("CAD", "Stroke"), c("CAD_assay_score", "Stroke_assay_score"))
fwrite(avg_scores_wide, sep="\t", quote=FALSE, file="analyses/assay_score_training/coef_avg_assay_scores.txt")

# For each score, how many coefficients are included across N training runs
possible <- assay_scaling[,unique(biomarker)]
...
setDT(coef_stats)
coef_stats[assay_coef[type == "assays",.N,by=.(endpoint, sex, lambda.fit, coef)], on = .(endpoint, sex, lambda.fit, coef), N_scores := i.N]
coef_stats[assay_coef[type == "assays",.(N=sum(round(beta, digits=3) != 0)), by=.(endpoint, sex, lambda.fit, coef)], on = .(endpoint, sex, lambda.fit, coef), N_scores_sig := i.N]
coef_stats[, N_score_sig_incl_avg := N_scores_sig]
coef_stats[!avg_assay_coef[type == "assays" & round(beta, digits=3) != 0], on = .(endpoint, sex, lambda.fit, coef), N_score_sig_incl_avg := 0]
...
g <- ggplot(ggdt) + 
  ...
  scale_y_continuous("Number of coefficients", limits=c(0, length(possible))) +
  theme_bw() + 
  theme(
    axis.text=element_text(size=6), axis.title=element_text(size=8),
    strip.text=element_text(size=8, face="bold"), strip.background=element_blank(),
...
ggsave(g, width=7, height=5.5, file="analyses/assay_score_training/coefficient_inclusion_matrix_assay_scores.pdf")
...
ggsave(g, width=7, height=5.5, file="analyses/assay_score_training/sig_coefficient_inclusion_matrix_assay_scores.pdf")
...
ggsave(g, width=7, height=5.5, file="analyses/assay_score_training/sig_avg_coefficient_inclusion_matrix_assay_scores.pdf")
...
coef_plot <- function(this_endpoint, this_sex, this_type, this_lambda="lambda.min", round=FALSE) {
  ggdt_ribbon <- avg_assay_coef[endpoint == this_endpoint & sex == this_sex & lambda.fit == this_lambda & type == this_type]
  if (round) {
    ggdt_ribbon[, beta := round(beta, digits=3)]
    ggdt_ribbon <- ggdt_ribbon[beta != 0]
  } 

  ggdt_points <- assay_coef[endpoint == this_endpoint & sex == this_sex & lambda.fit == this_lambda & type == this_type]
  if (round) {
...
g1 <- coef_plot("CAD", "Male", "assays")
g2 <- coef_plot("CAD", "Female", "assays")
g3 <- coef_plot("Stroke", "Male", "assays")
g4 <- coef_plot("Stroke", "Female", "assays")
g <- plot_grid(g1, g2, g3, g4, ncol=1, vjust=1.5, hjust=-0.2, label_size=8, align="hv",
  labels=c(
    "Assay score for CAD in males               ", 
    "Assay score for CAD in females             ", 
    "Assay score for ischaemic stroke in males  ", 
    "Assay score for ischaemic stroke in females"
  ))

ggsave(g, width=14, height=8, file="analyses/assay_score_training/coefficient_consistency_assays.pdf")

g1 <- coef_plot("CAD", "Male", "assays", round=TRUE)
g2 <- coef_plot("CAD", "Female", "assays", round=TRUE)
g3 <- coef_plot("Stroke", "Male", "assays", round=TRUE)
g4 <- coef_plot("Stroke", "Female", "assays", round=TRUE)
g <- plot_grid(g1, g2, g3, g4, ncol=1, vjust=1.5, hjust=-0.2, label_size=8, align="hv",
  labels=c(
    "Assay score for CAD in males               ", 
    "Assay score for CAD in females             ", 
...
ggsave(g, width=14, height=8, file="analyses/assay_score_training/sig_coefficient_consistency_assays.pdf")

# Make density plot of coefficients
ggdt <- assay_coef[lambda.fit == "lambda.min" & type == "assays"]
...
ggsave(g, width=7.2, height=6, file="analyses/assay_score_training/coefficient_denisities_assays.pdf")

# Write out table of coefficients for supp
dt <- avg_assay_coef[lambda.fit == "lambda.min" & type == "assays"]
dt[assay_scaling, on = .(coef=biomarker, sex), c("scaling_mean", "scaling_sd") := .(i.mean, i.sd)]
...
fwrite(dt, sep="\t", quote=FALSE, file="analyses/assay_score_training/supp_table_coefficients.txt")
